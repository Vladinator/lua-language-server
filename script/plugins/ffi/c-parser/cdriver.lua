...
function cdriver.process_file(filename)
    ...
    if not ctx then
        ---@diagnostic expect-next-line: need-check-nil -- `err` is set whenever `ctx` is nil
        return nil, "failed preprocessing '"..filename.."': " .. err
    end

    local srccode = table.concat(ctx.output, "\n").." $EOF$"

    local res, line, col, fragment
    res, err, line, col, fragment = c99.match_language_grammar(srccode)
    if not res then
        return nil, ("failed parsing: %s:%d:%d: %s\n%s"):format(filename, line, col, err, fragment)
    end

    local ffi_types
    ffi_types, err = ctypes.register_types(res)
