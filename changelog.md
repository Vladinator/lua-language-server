...
* `FIX` `need-check-nil` is no longer reported after `while true do ... end` for a variable that the loop assigns before its first `break`, since the loop is left only through its `break`s.
