...
* `NEW` `---@generic T = Default` declares a default type for a generic (TypeScript's `<T = Default>`): used when nothing infers `T` (no argument typed `T` was given, or it was `nil`), instead of `unknown`. An argument that does infer `T` still wins over the default. `---@generic K, V = string` gives `V` the default, not `K`.
* `FIX` `---@generic` names separated by `=` (a default type) now tokenize correctly; `=` was missing from the LuaDoc comment lexer's set of recognized symbols.
* `NEW` `---@param t readonly T` / `---@type readonly T`: the value that slot holds must not be mutated through that reference. The new `mutate-readonly` diagnostic reports assigning one of its fields/indices (`t.x = v`, `t[k] = v`), or passing it to a mutating stdlib call (`table.insert`, `table.remove`, `table.sort`, `rawset`). Only a direct reference is followed (not an alias, not what a call returns, not `table.move`); a style that needs more is silenced with `---@diagnostic disable-next-line: mutate-readonly`.
* `NEW` `unchecked-array-index` diagnostic (off by default): reports reading an element of a `T[]` array (`arr[i]`) in a context that treats the value as definitely not `nil` (arithmetic, further indexing, being called, a numeric `for` bound). A Lua array has no compile-time length, so most such reads are guarded by something the checker cannot see; turning this on finds candidates to review, not a default-on check.
...
* `NEW` `@secret` accepts a list of names (`---@secret a, b`), `---@secret-unwrap` declassifies a value (with the `redundant-secret-unwrap` and `undefined-secret-name` diagnostics), and `secret` is a type keyword (`---@param token secret string`). Completion offers the tags and keywords the plugins register, and completes the attributes of `---@class (` / `---@alias (` / `---@enum (`, the visibility keywords of `---@field`, and the locals of `---@secret a, b`. Hover shows the description of a registered tag and of an attribute.
* `NEW` `---@param str nosecret string` declares that a parameter cannot take a secret value (an API that raises an error for one), and the new `secret-argument` diagnostic reports a call that passes a value known to be secret there. Checked values (`---@secret-check`) are not reported, a parameter without the keyword takes anything, and with overloads only a call that none of them takes is reported. Also works in `fun(name: nosecret string)`.
* `FIX` Type guards and `---@return never` did nothing in a file that is not part of a workspace (VS Code with a single file open): the functions that declare them were only looked up in the workspace's files, not in the file itself.
* `FIX` Narrowing by a literal field (`if s.kind == 'circle' then`) also works for a union of table types written inline (`{ kind: 'circle', r: number } | { kind: 'square', side: number }`) and for several classes that share the literal, not only for one `---@class` per literal. A member that declares several literals (`kind: 'a'|'b'`) stays in the branch where either is possible. Together with `never` this makes the exhaustiveness check work for such unions.
* `FIX` Nothing but `never` itself can be assigned to a `never` slot (a table used to pass as a class without fields).
* `NEW` `---@return never` on a function that never returns because it always raises an error (`fail()`, `panic()`): a call of it ends the block like `error(...)` does. `missing-return` is not reported after it (nor for the function itself), `unreachable-code` sees code after branches that all end in it, `if not x then fail() end` leaves `x` non-nil, and `x or fail()` is `x` without nil. Works for local functions, `M.f()` and `M:f()`, also from another file.
* `NEW` `---@field readonly name string` declares a field that is set when the object is built and not changed after (TypeScript's `readonly`), and the new `assign-readonly` diagnostic reports assigning it (`obj.name = v`, `obj['name'] = v`, also for an inherited field). Not reported: the table constructor, anything inside a function named `new`, `init`, `constructor`, `ctor`, `__init` or `create`, and a local of the same function made from a table constructor or `setmetatable(...)`. Anything else that builds objects is silenced with `---@diagnostic disable-next-line: assign-readonly`.
* `NEW` Type guards, like TypeScript's `x is T`: `---@guard v is string` on a function that returns a boolean (`---@guard v is not nil` the other way round) narrows the argument in the branch of a condition where the call holds and takes the type away in the other; `---@asserts v is table` on a function that raises an error unless the check holds narrows it after a call used as a statement. `v` is a parameter (`self` too), the callee can be a local, a global, `M.f`, `M:f` or a function of another file. The new `invalid-guard` diagnostic reports a tag that is not of that form. In the plugin API: `registerGuardTag`, and call-narrowing rules can say `statement = true`.
* `NEW` `never`, the type of a value that cannot exist, is a built-in type name. Meant for checks, not for declaring variables: after an `if` / `elseif` chain over `type(x) == ...` or over a literal field of a class union (`s.kind == 'circle'`), `---@type never` and `local _exhaustive = x` in the last `else` is reported by `assign-type-mismatch` when a variant is still possible there.
* `NEW` Diagnostic registrations can say `narrowSettings` (settings only that diagnostic reads: a change runs just those diagnostics), `afterAll` and `fullRunWhen` (a diagnostic that needs the outcome of all the others, like `unfulfilled-expect`). The built-in diagnostics use them instead of tables in the provider.
* `NEW` `---@return nosecret string` / `---@nosecret` (above a function, every return) declare that a function must not return a secret value, `---@type nosecret string` / `---@nosecret [names]` that a local cannot hold one. The new `secret-return` and `secret-variable` diagnostics report a `return` / an assignment of a value known to be secret. `---@secret` and `---@nosecret` on the same function or local contradict each other and are reported on the `nosecret` tag; `undefined-secret-name` also checks the names in `---@nosecret a, b`. Only known secrets are reported (an unannotated callee is unknown), and a call result is not cleared by the tag: unwrap it with `---@secret-unwrap`.
* `FIX` `t.x = t.x or {}`: the read of `t.x` on the right is the field as it was before the assignment (it was narrowed by the assignment itself, in some orders), and a table type with optional fields (`{ text?: string }`) shows the `?` whatever was compiled first.
* `FIX` A result that was computed from a half-built node in a chain of assignments (`pos = pos + 1` several times in a loop, one of them after a call that reads `pos`) is no longer kept: the read after it was `unknown` in some orders (editor vs command line).
* `FIX` After `while cond do ... break ... end` a variable has the state it had at the `break`, not only the state where the condition is false (`while n do break end; print(n)` was `unknown` for a non-nil `n`).
* `FIX` The type of a variable that is assigned from itself in a loop (`n = n + 1`, `s = s .. x`) no longer depends on which of its reads is asked for first (it could be `unknown` in the editor while the command line check found the type).
* `FIX` `repeat ... until` filter reads of the loop variable no longer depend on compile order (`repeat pos = f(pos) until not pos or done`: the `until` read could be stuck at the type the variable had on entry, instead of what the body leaves it, when something else compiled the body's own reassignment first).
* `NEW` `---@field name nosecret string` declares that a field cannot hold a secret value, and the new `secret-field` diagnostic reports assigning one (`obj.name = v`, `obj['name'] = v`, or a table constructor typed as the class), also for a field inherited from a parent class.
* `NEW` The user plugin API (`Lua.runtime.plugin`) is documented in `doc/en-us/plugin.md`, and how to write a diagnostic plugin (`core/diagnostics/extra/`, `Lua.diagnostics.pluginsDir`) in `doc/en-us/diagnostic-plugin.md`. A diagnostic can list the settings it reads (`reads` in `proto.diagnostic.register`) so that a change of one runs it again.
* `FIX` User plugins: an error in a plugin was reported as `nil` and the message had no room for it; a plugin without a hook stopped the plugins after it; a later plugin that returned nothing wiped an earlier plugin's result; one plugin that failed to load skipped the rest; `OnTransformAst` returning something that is not a tree, or an error in `VM.OnCompileFunctionParam`, broke the file.
* `FIX` Changing a diagnostics setting (`Lua.diagnostics.globals`, `globalsRegex`, `disable`, `severity`, `neededFileStatus`, the group settings, `unusedLocalExclude`, `Lua.spell.dict`) no longer restarts the diagnosis of the whole workspace from its first file: only the diagnostics that read the setting run again and the others keep their cached results, a diagnosis that is running is not cancelled for it (the next one does what was asked), and what a cancelled diagnosis had still to do is not lost. Any other setting behaves as before. In a large workspace a client that writes settings while the diagnosis runs (an extension adding to `Lua.diagnostics.globals`) no longer keeps it from ever getting past the first files.
* `FIX` A library folder inside the workspace (for example the workspace's own `Lua.workspace.library` pointing at one of its subfolders) is no longer counted and loaded twice: the progress said `4022` files for 3508 and the files were read twice (loading that folder took 17 s, now 9 s).
* `FIX` `missing-fields` is about three times faster in a file that builds many tables of one class (it worked out the required fields of the class again for every table constructor): 28 s to 10 s of a 90 s workspace diagnosis of 3,500 files, the same 514 findings.
* `NEW` `LLS_DIAG_PROFILE=<file>` (environment variable of the server) writes what each diagnostic has cost so far to the file, for finding out why a workspace is slow to diagnose.
* `FIX` `need-check-nil` is no longer reported after `while true do ... end` for a variable that the loop assigns before its first `break`, or initialises with `if not x then x = {} end` (also `x == nil`) before it, since the loop is left only through its `break`s. A `goto` to a label after the last `break` (the usual `continue`) no longer switches this off.
